...
- **لایه داده واقعی Binance** (V1.3.3): aggTrades برای CVD واقعی + depth برای اسپرد واقعی
- **Paper Trading** (V1.3.3): ثبت سیگنال و ارزیابی نتیجه بدون ریسک واقعی
- 96 تست واحد (همه موفق)

### 🔖 مدیریت نسخه
- نسخه‌بندی معنایی V1.3.3
- ساخت خودکار فایل زیپ با هر انتشار
- نگهداری حداکثر 3 نسخه اخیر
- CHANGELOG کامل با تاریخچه همه نسخه‌ها

### ⚠️ شفافیت محدودیت‌ها (V1.3.3)
...
3. **نسخه‌بندی**: V1.3.3 (Major.Minor.Patch)
...
# اسکن با تحلیل‌های پیشرفته (V1.3.3)
python main.py --advanced --limit 10 --format detailed

# بک‌تست سیگنال‌های گذشته (V1.3.3)
python main.py --backtest

# فقط هشدار حجم غیرعادی (V1.3.3)
...
│   ├── test_advanced_analysis.py  # V1.3.3
│   ├── test_rate_limiter.py       # V1.3.3
│   ├── test_backtest.py           # V1.3.3
│   └── test_volume_alert.py       # V1.3.3
└── src/
    ├── __init__.py
    ...
    ├── rate_limiter.py           # V1.3.3 - Token Bucket
    ├── indicators.py             # RSI، MACD، MA، ATR، Bollinger
    ├── advanced_analysis.py      # V1.3.3 - لیکوییدیتی/سوییپ/اردرفلو/والیوم پروفایل
    ├── analyzer.py               # ترکیب و صدور سیگنال (7 فاکتور)
    ├── backtest.py                # V1.3.3 - بک‌تست سیگنال‌ها
    ├── volume_alert.py            # V1.3.3 - هشدار حجم غیرعادی
    ├── reporter.py               # قالب‌بندی خروجی متنی
    ├── html_reporter.py          # V1.3.3 - داشبورد HTML
...
```bash
# رفع خطا
python scripts/release.py patch          # V1.3.3 → V1.3.3

# ویژگی جدید
python scripts/release.py minor          # V1.3.3 → V1.3.3

# تغییر بزرگ
python scripts/release.py major          # V1.3.3 → V1.3.3
